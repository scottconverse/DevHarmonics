...
**All eight spec slices implemented and live-fire accepted** (2026-08-05). CI runs the full suite green on Windows and Ubuntu (one POSIX-only probe is skipped on Windows; the exact test count changes too fast to quote — see CI). Every slice was accepted against real tools on a real machine — real provider CLIs, real local model endpoints, real diffs independently verified — never shim-only green.
...
- **Verification plane** — the [rigor-suite](https://github.com/scottconverse/rigor-suite), used but never bundled, so the tools that judge the work stay independent of it. Honest status of the three, as of 2026-08-05: **tampercheck** is a real runtime dependency — resolved from `PATH` (or an absolute path with an optional sha256 pin), executed at every integration boundary, and pinned into each target repo's CI by `onboard`; **deterministic-detector** is wired in two places — `onboard`'s `ci-detectors` step installs its CI workflow, and every `run`/`set` labels validator evidence by whether the target repo carries it (presence-only: it cannot read CI verdicts, and it never enforces); **dev-rigor-stack-lite** is coordinator discipline in markdown, so it is never executed, only version-checked for parity across hosts by `doctor`. The factory builds no verification code of its own.
