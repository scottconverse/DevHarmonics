...
| `admission.mjs` | Append-only `usage.jsonl` ledger — reservation, replay-based summing, reconciliation (ported from codex-factory), plus the **fan-out ceilings** (`admitWorker`/`reconcileWorker`/`summarizeFanout`): hard, fail-closed caps on total workers and cumulative tokens per state root within a rolling window, enforced inside both worker lanes before any spawn. The money half is live too: every credentialed Messages call reserves against `budgets.maxPaidTokens` before the request and reconciles with real usage after (`messages-client.mjs`); unknown paid usage closes the paid lane. |
...
| **Providers** | Any ACP-speaking adapter the operator installs; the code takes an arbitrary `adapterCommand`/`adapterArgs`, it does not hardcode which adapters exist | Ollama (`127.0.0.1:11434`) and LM Studio (`127.0.0.1:1234`) — `config.mjs`'s defaults — plus any operator-added Anthropic-dialect endpoint (e.g. a LiteLLM proxy translating for OpenAI-format backends) and the real Claude API as an explicit opt-in | `codex` (`codex exec`), `claude` (`claude -p`, non-bare), `agy` (Antigravity's `agy -p` Command Mode) — the full set is `providers.mjs`'s `SUBPROCESS_PROVIDERS` |
...
| Two-phase atomicity in sets (`integration-set.mjs`, `docs/INTEGRATION-SETS.md`) | Sets are genuinely all-or-nothing: phase 1 prepares every member — all gates run, the merged candidate is built and parked on a durable temp ref — with **no** integration ref moving; refs advance only in phase 2, after every member passed. A refused member means nothing advances anywhere (fully-gated siblings report `set-blocked-not-advanced`), a branch that moved between phases refuses as `integration-ref-moved`, and a finalize failure rolls already-advanced refs back to their recorded pre-merge positions. This replaced the earlier honest-but-non-atomic `advanced-but-set-blocked` behavior after a 2026-08-05 audit rated it a defect for the cross-repo path. |
