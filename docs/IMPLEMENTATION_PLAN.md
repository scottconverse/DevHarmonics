...
Revision history: **v1.33 (2026-07-22)** — Item-6 (DH-810) design decisions locked at PLAN time, before code. A workflow is a **versioned, parameterized document stored in Git** under the product repository (`.devharmonics/workflows/*.json`), identified by content hash — the same content-addressed discipline the ledger already applies to review evidence bindings (plan revisions, by contrast, use per-objective counters; the shared property is immutability of the record, not the identifier scheme) — and running a workflow pins that hash into the run record once, immutably, so a later edit can never rewrite what a historical run executed. Execution REUSES the objective composer: instantiating a workflow with typed inputs produces an objective draft and plan proposal through the existing propose/approve/start path — no second execution engine. Approval points map to the existing approval machinery; a template promotion that would widen permissions relative to the promoted pilot is refused rather than silently accepted. First two concrete workflows ship as fixtures-of-record: documentation-consistency and release-truth audit. Ledger migration 30 adds workflow revision persistence and run linkage.
