...
- To use the **HTTP** lane: a locally running Anthropic-compatible Messages endpoint. The built-in defaults are Ollama on `http://127.0.0.1:11434` and LM Studio on `http://127.0.0.1:1234` — both speak the Anthropic Messages dialect natively. A backend that speaks only the OpenAI format (vLLM, a llama.cpp server, cloud aggregators) connects through a translating proxy such as LiteLLM: run the proxy, then add `"litellm": { "baseUrl": "http://127.0.0.1:4000" }` under `endpoints` in the project config (which materializes on first use — see the `config` command) and use `--provider litellm`. A real Anthropic API key is supported only as a separate, explicit opt-in — nothing reaches for one by default.
...
What it actually checks: each configured CLI (`codex`, `claude`, `agy`) by resolving it on `PATH` and running its real `--version`; whether you are actually **signed in** to each of those providers — a separate question from "installed", asked by running the provider's own status command (`codex login status`, `claude auth status --text`, `agy models`) with credential-shaped environment variables stripped first, so an API key sitting in your environment can never make a signed-OUT subscription look signed in. A signed-out provider FAILs with the exact command **you** run to fix it; DevHarmonics never asks for, sees, or stores a password, and whatever the provider prints back is redacted and length-capped before it reaches the report. Then: each configured HTTP endpoint (Ollama and LM Studio by default) by discovering an available model and sending it a real Messages request; whether `tampercheck` itself is on `PATH`; whether the `dev-rigor-stack-lite` skill is installed at the *same* version under every configured coordinator host (`~/.claude/skills` and `~/.codex/skills` by default); and — only when the config declares a credentialed endpoint — a `paid:<name>` row that says in plain language whether a paid call would actually work: FAIL when the named environment variable is not set (or the named stored credential is not in the store), FAIL when `budgets.maxPaidTokens` is missing ("every paid call will refuse"), FAIL when `budgets.allowPaidApi` is not `true` (the double opt-in's master switch), and PASS stating the ceiling in tokens with a clearly-labeled approximate dollar range — plus the USD ceilings, when the pair is configured. Misconfiguration surfaces here, in the diagnostic, not in a refused run.
...
- `--base-url <url>` — the HTTP endpoint to call. Only meaningful for `--lane http`. If omitted, DevHarmonics looks up `endpoints.<provider>.baseUrl` in the configuration (resolved the usual way: explicit `--config` > the project config > defaults) — so `--provider ollama` and `--provider lmstudio` work out of the box without `--base-url`, and any endpoint you add to the config (e.g. a LiteLLM proxy) works by its name too; any other provider name needs an explicit `--base-url` or the command errors out.
...
**The ledger looks after itself.** `usage.jsonl` is append-only, so it would otherwise grow forever and slow every run down. Once it passes about 5 MB, DevHarmonics compacts it automatically the next time it writes — but never by throwing history away, because silently dropping records would silently reset your ceilings. Compaction keeps every record that can still affect a live decision (anything inside your fan-out window, and anything it cannot date), preserves your lifetime paid total exactly as a single carry-forward record, keeps a one-line marker for every archived run so a task ID stays single-use forever, and writes everything it removed to a dated `.archive` file next to the ledger — nothing is deleted. If any of what it would archive is untrustworthy, or if compacting wouldn't actually reclaim space, it doesn't happen at all and the run continues against the full ledger.
...
- **Reservation before the request**: each paid call reserves an estimate (the response ceiling you asked for plus an estimate of the prompt) against `maxPaidTokens`; if the ledger's recorded spend plus that reservation would exceed the cap, the call is refused (`paid-budget-exceeded`) and the request is never sent. **Honestly: this is an overage stop, not a pre-flight guarantee** — the same caveat that applies to `--max-budget-usd`. The prompt estimate is exactly that, an estimate, so a single call can cross the ceiling before reconciliation records what it really cost. The estimate deliberately errs high (about two characters per token) so overshoot is small and rare, and every subsequent call sees the true recorded total.
- **Reconciliation after**: the reservation is replaced by the response's *real* token usage. Records are **write-once in effect**: if more than one final record exists for the same call, the **highest** reported figure counts. A duplicate write therefore can't double-charge you, and no later line can quietly lower what's already recorded. Correcting a charge downward requires deliberately rotating the ledger (below) — a visible act, never a silent append.
...
**Dollar ceilings** (`budgets.perRunLimitUsd` and `budgets.monthlyLimitUsd`) sit on top of the token ceiling, and they come **as a pair** — configuring one without the other is a validation error, both must be positive (v1's rule, kept). They are enforced on **real, reported cost only — never a price table**:

- `perRunLimitUsd` rides to the one provider that enforces a genuine per-run dollar stop: `claude`'s own `--max-budget-usd`. It applies automatically to every `run` and `worker` invocation that uses the `claude` provider; if you also pass `--max-budget-usd` explicitly, the **lower** of the two wins, so the config ceiling can never be talked past on the command line.
- `monthlyLimitUsd` is summed from the cost your workers actually *reported* (claude's headless receipts carry a real `total_cost_usd`) over a rolling 30 days, on the same admission ledger as the fan-out ceilings. Once recorded spend reaches the limit, the next worker is refused before it spawns (`paid-monthly-usd-exceeded`).

Stated honestly: a run that reports no cost — every local model, and raw Messages-API calls, which return token usage but no dollar figure — contributes **$0** to the monthly sum. The USD ceilings guard the spend that is *verifiable*; the token ceilings (`maxPaidTokens`, `maxTotalTokens`) are the universal floor that guards everything, because tokens are what every response actually reports. DevHarmonics will never estimate your dollars from a price table and enforce the guess.

Everything key-less — the default local Ollama/LM Studio endpoints, any keyless endpoint you add (e.g. a LiteLLM proxy), the subscription CLIs — is unpaid and untouched by any of this. Two knowingly-unwired corners: `doctor`'s endpoint probe and the `tool_use` qualification probe always send the local placeholder key, so they cannot probe a credentialed endpoint (they'll get an auth error rather than spending money — the safe direction). Everything fails closed: a refusal is an honest failed receipt naming the ceiling and the ledger (`fanout-workers-exceeded`, `fanout-tokens-exceeded`, `fanout-concurrency`), and a ledger that cannot be read refuses admission (`fanout-admission-unavailable`) rather than waving the worker through. There is no off switch — only deliberately raised limits.
