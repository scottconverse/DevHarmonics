...
- The `--check` validator's own subprocess — whether it's the subprocess/acp lane's separate check step, or the check command `local-patch.mjs` runs internally for the http lane — receives your ordinary, **unstripped** environment. The boundary covers the worker's own child process; it was never meant to cover every process the pipeline spawns.
