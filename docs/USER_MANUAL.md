...
- `--timeout-minutes` (default 15) bounds the worker on all three lanes (for the http lane it covers the model call and the internal check that `local-patch.mjs` runs) and, if requested, the reviewer stage. The value is validated at parse time: anything that is not a positive, finite number of minutes (`nope`, `-1`, `Infinity`) is a flag error (exit 2), never a NaN or infinite timeout reaching a subprocess. The `--check` validator step on the subprocess/acp lanes and the `tampercheck` gate each use their own fixed internal timeouts (10 minutes and 2 minutes) regardless of this flag.
- `--max-budget-usd <n>` — a spend ceiling forwarded to a provider CLI that supports one (today that is `claude`, as its own `--max-budget-usd` flag; the other providers have no equivalent and the value is not sent to them). Validated at parse time the same way as `--timeout-minutes`. This bounds one worker invocation's spend at the provider side; it is not a cross-run budget ledger.
...
An adversarial test (documented in full in `docs/FALSIFICATION.md`, finding F-1) substituted a fake `tampercheck` earlier on `PATH` that always printed a clean line and exited 0; a change that skipped a failing test was integrated as a result. The fix — described in step 5 above — is implemented in `integrateWorkerBranch` and is **on by default today**, automatically, for both `run` and `set`; there is no flag on either command that needs to be set to turn it on. It checks *shape*, not an exact value, on purpose: pinning an exact expected version was tried and rejected, because it would lock out an operator legitimately running a different `tampercheck` release while adding no real security (a stub author can print whatever version string is expected). Stronger binding than the shape check exists, but is library-only — reachable by calling `integrateWorkerBranch` directly rather than through any `devharmonics` command: an absolute `tampercheckPath` (skipping `PATH` resolution entirely) and an `expectedTampercheckSha256` content pin — the one local check a stub author cannot satisfy by printing an expected string; a mismatch refuses as `tampercheck-unavailable`. Also library-only: pinning an *exact* expected version (`expectedTampercheckVersion`), and turning the identity check off entirely (`requireTampercheckIdentity: false`). Whether or not you pin, the resolved binary's path and SHA-256 checksum are always recorded in the integration evidence, so a receipt can say which binary actually answered. The threat this defends against is bounded: a worker child process cannot alter its parent's `PATH`, so this is an operator-environment risk, not something a worker can exploit — and a second, independent defense already exists regardless: `devharmonics onboard` installs a repository-side CI workflow that installs its own pinned `tampercheck` fresh on every run, unaffected by anything on the local operator's `PATH`.
...
- **`run --check` executes in the worker's own worktree, before the merge — not on the merged candidate.** `set --check` runs each member's validator against the exact merged candidate that would be delivered; `run`'s validator runs earlier, on the worker's tree, so in principle `run` can pass a check that the delivered merge would fail. (The final-artifact `tampercheck` scan does run on the merged commit for both commands.) A known asymmetry, tracked to be closed.
