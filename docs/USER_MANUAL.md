...
- `maxConcurrentWorkers` (default 3) — how many workers may be *live at once* (1–4). A full house means the next worker **waits** for a free slot (bounded by its own `--timeout-minutes`), so work serializes rather than oversubscribing; it never means an extra worker runs anyway.
...
All three are metered per **state root** — the `.devharmonics` directory nearest the worker's runs — over a rolling window (`windowHours`, default 24), recorded in an append-only `usage.jsonl` ledger there: a reservation line before each spawn, a terminal line with the real reported usage after. Old entries age out of the window on their own, so a tripped ceiling clears with time; to clear it deliberately, raise the cap in config or rotate the ledger file — both are visible, deliberate acts. Everything fails closed: a refusal is an honest failed receipt naming the ceiling and the ledger (`fanout-workers-exceeded`, `fanout-tokens-exceeded`, `fanout-concurrency`), and a ledger that cannot be read refuses admission (`fanout-admission-unavailable`) rather than waving the worker through. There is no off switch — only deliberately raised limits.
