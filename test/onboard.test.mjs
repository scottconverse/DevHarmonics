...
test("planOnboarding on a fresh git repo reports every step missing and never writes", () => withRepo(async (repo) => {
  ...
  const plan = planOnboarding({ repository: repo });

  assert.equal(plan.repository, repo);
  assert.equal(plan.steps.length, 2, "no README.md in this fixture -> readme-badge must not be offered");
...
test("applyOnboarding writes the pinned workflow and the exclude entry; re-planning afterward reports present", () => withRepo(async (repo) => {
  ...
  const rePlan = planOnboarding({ repository: repo });
  for (const step of rePlan.steps) assert.equal(step.status, "present", `${step.id} should be present after apply`);
  ...
  assert.deepEqual([...reApply.skipped].sort(), ["ci-tampercheck", "gitignore-devharmonics"]);
...
test("readme-badge: not offered without a README; missing then applied with exactly one appended line; present once there", () => withRepo(async (repo) => {
  ...
  const noReadmePlan = planOnboarding({ repository: repo });
...
test("onboardCommand dry run writes nothing; --apply writes; --repository also works; bad flags and non-git paths are runner errors", () => withRepo(async (repo) => {
  ...
  const before = git(repo, ["status", "--porcelain"]);
  const dryExit = await onboardCommand([repo], { write: dry.write });
  ...
  const applyExit = await onboardCommand([repo, "--apply"], { write: apply.write });
  ...
  const alreadyExit = await onboardCommand(["--repository", repo, "--apply", "--json"], { write: already.write });
  assert.equal(alreadyExit, 0);
  const parsed = JSON.parse(already.text());
  assert.deepEqual(parsed.applied, []);
  assert.deepEqual([...parsed.skipped].sort(), ["ci-tampercheck", "gitignore-devharmonics"]);
  ...
  await onboardCommand([repo], { write: dryDiffers.write });
  assert.ok(dryDiffers.text().includes("differs"));

  const applyNoForce = collectWrites();
  await onboardCommand([repo, "--apply"], { write: applyNoForce.write });
  ...
  await onboardCommand([repo, "--apply", "--force"], { write: applyForce.write });
  assert.equal(readFileSync(workflowPathOf(repo), "utf8"), renderTampercheckWorkflow(TAMPERCHECK_PINNED_VERSION));
